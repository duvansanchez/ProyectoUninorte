...
def activate():
    try:
        ...
        if request.method == 'GET': 
            #Obtener la autenticacion
            number = request.args['auth'] 
            
            db =  get_db()
            #Realizar una consulta con el proposito de comprobar el estado y el tiempo de creacion del link de activacion y verificar si el estado de tiempo es valido
            attempt = db.execute(
            ...
            if attempt is not None:
                #Actualizar el estado correspondiente a el id dado en la tabla activationlink
                db.execute(
                    'UPDATE activationlink SET state = ? WHERE id = ?', (utils.U_CONFIRMED, attempt['id'])
                )
                #Insertar datos del usuario luego de realizar previamente la consulta y actualizacion del estado de inactivo a activo
                db.execute(
...
def register():
    try:
        ...
        #Obtener los valores de los campos indicados y realizar la validacion de cada uno de ellos
        if request.method == 'POST':    
            username = request.form['username']
            password = request.form['password']
            email = request.form['email']
            
            db =  get_db() #Obtener instancia de la base de datos
            error = None
            ...
            if not password:
                ...
                flash(error)
                return render_template('auth/register.html')
            #Ejecutar consulta con el proposito de verificar si ya hay un usuario registrado con el username dado
            if db.execute('SELECT id FROM user WHERE username = ?', (username,)).fetchone() is not None:
                error = 'User {} is already registered.'.format(username)
                flash(error)
                return render_template('auth/register.html')
            #Verificar validaciones del email
...
def confirm():
    try:
        ...
        if request.method == 'POST': 
            ...
            if attempt is not None:
                #Actualizar el estado correspondiente a el id dado
                db.execute(
                    'UPDATE forgotlink SET state = ? WHERE id = ?', (utils.F_INACTIVE, attempt['id'])
                )
                #Crear llave aleatoria y luego juntarla junto a esta contraseña
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                #Actualizar contraseña, llave criptografica del respectivo id
                db.execute(
